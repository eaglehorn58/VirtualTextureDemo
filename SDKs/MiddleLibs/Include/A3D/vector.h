 ...
 *
 * HISTORY: 可以使用ADyncArray替代，尽量不要使用了
 *
...
public:
	...
	//	added by dyx
	//	quick_clear() only reset _cur_size and _finish, but doesn't call destructors of current array items,
	//	so it may be only used for vector that stores simple type data (int, float, pointer, etc...).
	void quick_clear()
	{
		_finish = _data;
		_cur_size = 0;
