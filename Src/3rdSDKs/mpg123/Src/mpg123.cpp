...
int CMpg123::mpg123_seek(int nSample)
{
	...
	{
		m_readers->GetReader()->filept->seek(0, SEEK_END);
		return 1;
	}

	m_readers->GetReader()->filept->seek(GetFileOffset() + nFrame * (m_readers->GetFR()->framesize + 4), SEEK_SET);
	...
	*preadsize = 0;
	while(size > 0)
	{
		...
		else
		{
			...
			if( pcm_point <= 0 )
			{
				int nPos = m_readers->GetReader()->filept->pos();
...
CMpg123::CMpg123(CMpg123File* pFile/*NULL*/)
{
	pcm_sample = NULL;
	pcm_point = 0;
