...
public:
    ...
    StrPair() : _flags( 0 ), _start( 0 ),_len(0)  {}	//_end( 0 )
    ~StrPair();
...
public:
	...
	//	by raochao 2015.4.1
	inline static bool StringEqualNoCase( const char* p, const char* q, int nChar=INT_MAX ){
		if ( p == q ) {
			return true;
		}
		int nDelta = 'a' - 'A';
		int n = 0;
		while( *p && *q && (*p == *q || (*p >= 'a' && *p <= 'z' && *q >= 'A' && *q <= 'Z') && (abs(*p - *q) == nDelta)) && n<nChar ) {
			++p;
			++q;
			++n;
		}
...
public:
    ...
    */
    ...
    /**
    	...
    	an errorID.
    */
	//	使用我们自己的AXFile，否则FileFetcher以后没法用
    //XMLError LoadFile( FILE* );
	XMLError LoadFile(AFileWrapper* pFile);
    ...
    */
    ...
    /**
    	...
    	an errorID.
    */
	XMLError SaveFile(AFileWrapper* pFile, bool compact = false);
...
// 	static std::string trimBom(std::string & in);
// 	static std::string convertToUtf8(std::string & in);
...
// modify by raochao 2015.3.31，底层放弃FILE使用AXFile
...
public:
    ...
    */
	...
	XMLPrinter(AFileWrapper* pFile = 0, bool compact = false, int depth = 0);
    virtual ~XMLPrinter()	{}
