...
AString::AString(const AString& str)
{
	...
	*/
	const char* szStr = (const char*)str;
...
AString& AString::Format(const char* szFormat, ...)
{
	...
	int iNumWritten, iMaxLen = _vscprintf(szFormat, argList) + 1;
...
/*	Make a guess at the maximum length of the resulting string.
	now this function doesn't support UNICODE string. I64 modifier used in WIN32's
	sprintf is neither supported
...
AString& AString::Replace(const char* szFrom, const char* szTo)
{
	...
	aint32 fromlen = (int)strlen(szFrom);
	aint32 nToLen = (int)strlen(szTo);
	aint32 i = Result->Find(szFrom);
	while (i != -1)
	{
		*Result = Result->Left(i) + AString(szTo) + Result->Mid(i + fromlen);
		i = Result->Find(szFrom, i + nToLen);
	}
