...
static float _HitPointOfOBBAndPolygon(const AXOBB& OBB, AXVector3** aVerts, int iNumVert, const AXPLANE& Plane);
...
*/
bool AXCLS::RayToAABB3(const AXVector3& vStart, const AXVector3& vDelta, const AXVector3& vMins,
					const AXVector3& vMaxs, AXVector3& vPoint, float* pfFraction, AXVector3& vNormal)
{
	AXVector3 vN, vMaxT(-1.0f, -1.0f, -1.0f);
	bool bInside = true;
	int i;
	float aSign[3];
	...
	if (bInside)
	{
		vPoint		= vStart;
		*pfFraction = 0;
		vNormal		= AXVector3(0.0f);
		return true;
	}

	//	Get largest of the maxT's for final choice of intersection
	int iWhichPlane = 0;
	vN = AXVector3(-aSign[0], 0.0f, 0.0f);

	if (vMaxT.m[1] > vMaxT.m[iWhichPlane])
	{
		iWhichPlane = 1;
		vN = AXVector3(0.0f, -aSign[1], 0.0f);
	}

	if (vMaxT.m[2] > vMaxT.m[iWhichPlane])
	{
		iWhichPlane = 2;
		vN = AXVector3(0.0f, 0.0f, -aSign[2]);
	}
...
*/
bool AXCLS::RayToAABB2(const AXVector3& vStart, const AXVector3& vDelta, float vMins[2],
					float vMaxs[2], AXVector3& vPoint)
{
	float vMaxT[2] = {-1, -1};
	bool bInside = true;
	int i;
...
*/
bool AXCLS::RayToOBB3(const AXVector3& vStart, const AXVector3& vDelta, const AXOBB& OBB, 
				   AXVector3& vPoint, float* pfFraction, AXVector3& vNormal)
{
	float t1, t2, fExt, fDelta;
	float fMinT = -99999, fMaxT = 99999;
	AXVector3 vDist = OBB.Center - vStart;
	AXVector3 vN1, vN2;
...
*/
int CLS_PlaneToAABB(const AXVector3& vNormal, float fDist, const AXVector3& vOrigin,
					const AXVector3& vExtents)
{
	AXVector3 vMins, vMaxs;
...
*/
int AXCLS::PlaneAABBOverlap(const AXSPLANE& Plane, const AXVector3& _vMins, const AXVector3& _vMaxs)
{
	float d1, d2;
...
/*
__declspec( naked ) int AXCLS::PlaneAABBOverlap(const AXPLANE& Plane, const AXVector3& _vMins,
										const AXVector3& _vMaxs)
{
...
*/
int AXCLS::PlaneSphereOverlap(const AXSPLANE& Plane, const AXVector3& vCenter, float fRadius)
{
	float fDelta;
...
*/
bool AXCLS::TriangleAABBOverlap(const AXVector3& _v0, const AXVector3& _v1, const AXVector3& _v2, 
						const AXSPLANE& Plane, const AXAABB& AABB)
{
	//	First translate triangle into AABB's coordinates
	AXVector3 v0(_v0 - AABB.Center);
	AXVector3 v1(_v1 - AABB.Center);
	AXVector3 v2(_v2 - AABB.Center);
	...
	if (AXCLS::PlaneAABBOverlap(Plane, AABB.Mins, AABB.Maxs))
		return false;
...
*/
bool AXCLS::RayToTriangle(const AXVector3& vStart, const AXVector3& vDelta, const AXVector3& v0,
						const AXVector3& v1, const AXVector3& v2, AXVector3& vPoint, bool b2Sides, float* pfFraction)
{

	//	Plane: Dot((a, b, c), (x, y, z)) + d = 0
	//	Line: p + vt = 0
	//	Dot(p+vt, N) + d = 0
// 	if (pfFraction)
// 	{
...
// 	}
	...
	AXVector3 vEdge0 = v1 - v0;
	AXVector3 vEdge1 = v2 - v0;
	AXVector3 N = CrossProduct(vEdge0, vEdge1);
	//	三角形面积趋近于0
	...
	if (!b2Sides && DotProduct(N, vDelta) > EPSILON)
	{
		return false;
	}

	float d = -DotProduct(N, v0);
	float det = DotProduct(vDelta, N);
	//	三角形与射线平行
	if (det < EPSILON && det > -EPSILON)
	{
		return false;
	}
	float t = -(DotProduct(vStart, N) + d) / det;
	// 射线反方向或延长线与平面相交
	if (t < EPSILON || t >= 1.0f)
	{
		return false;
	...
	AXVector3 vIntersection = vStart + vDelta * t;
	AXVector3 vInnerLine0 = vIntersection - v0;
	AXVector3 N0 = CrossProduct(vEdge0, vInnerLine0);
	if (DotProduct(N0, N) < -EPSILON)
	{
		return false;
	}
	AXVector3 N1 = CrossProduct(vInnerLine0, vEdge1);
	if (DotProduct(N1, N) < -EPSILON)
	{
		return false;
	}
	AXVector3 N2 = CrossProduct(vIntersection - v2, v1 - v2);
	if (DotProduct(N2, N) < -EPSILON)
	{
		return false;
	}

	vPoint = vIntersection;
	if (pfFraction)
	{
		*pfFraction = t;
	}
	...
	const AXVector3& v1, const AXVector3& v2, AXVector3& vPoint, bool b2Sides, float* pfFraction)
{
	...
	AXVector3 vDir = vDelta;
	float fDist = vDir.Normalize();

	//	Find vectors for two edges sharing vert0
	AXVector3 vEdge1 = v1 - v0;
	AXVector3 vEdge2 = v2 - v0;

	//	Begin calculating determinant - also used to calculate U parameter
	AXVector3 tvec, qvec;
	AXVector3 pvec = CrossProduct(vDir, vEdge2);
...
*/	
bool AXCLS::AABBAABBOverlap(const AXVector3& vCenter1, const AXVector3& vExt1, 
						 const AXVector3& vCenter2, const AXVector3& vExt2)
{
	float fDist;
...
*/
bool AXCLS::RaySphereOverlap(const AXVector3& vStart, const AXVector3& vDelta, const AXVector3& vOrigin, float fRadius)
{
	AXVector3 vDir, vTemp;
	float fLen, d;
...
*/
bool AXCLS::RayToPlane(const AXVector3& vStart, const AXVector3& vDelta, const AXVector3& vPlaneNormal,
					float fDist, bool b2Sides, AXVector3& vHitPos, float* pfFraction)
{
	AXVector3 vNormal = vPlaneNormal;
...
*/
bool AXCLS::OBBToQuadrangle(const AXOBB& OBB, const AXVector3& ov0, const AXVector3& ov1, const AXVector3& ov2,
						 const AXVector3& ov3, const AXVector3& vNormal, float* pfFraction)
{
	...
	AXVector3 _v0 = ov0 - OBB.Center;
	AXVector3 _v1 = ov1 - OBB.Center;
	AXVector3 _v2 = ov2 - OBB.Center; 
	AXVector3 _v3 = ov3 - OBB.Center;
	AXVector3 v0, v1, v2, v3;
...
*/
	...
	else
	{
		...
		AXVector3* aVerts[4] = {&v0, &v1, &v2, &v3};
...
*/
float _HitPointOfOBBAndPolygon(const AXOBB& OBB, AXVector3** aVerts, int iNumVert, const AXPLANE& Plane)
{
	AXVector3 aProj1[8], aProj2[8];	//	Vertices projection on x-y plane
	AXVector3 *v1, *v2, *aOld, *aNew;
	bool bIn1, bIn2;
...
*/
int AXCLS::AABBToPlane(const AXVector3& vStart, const AXVector3& vEnd, const AXVector3& vExts, 
					const AXSPLANE& Plane, float* pfFraction)
{
	AXVector3 vOffs;
...
/*	OBB-OBB overlap test using the separating axis theorem.
	...
	obb2: obb B
*/
bool AXCLS::OBBOBBOverlap(const AXOBB& obb1, const AXOBB& obb2)
{
	AXMatrix4 mat1, mat2;
	...
	AXMatrix4 mat = mat2 * mat1;

	return AXCLS::OBBOBBOverlap(obb1.Extents, obb2.Extents, mat);
}
...
*/
bool AXCLS::OBBOBBOverlap(const AXVector3& vExt1, const AXVector3& vExt2, const AXMatrix4& mat)
{
	AXMatrix4 am;
	...
	AXVector3 vT(mat._41, mat._42, mat._43);
...
*/
bool AXCLS::AABBOBBOverlap(const AXVector3& vCenter, const AXVector3& vExts, const AXOBB& obb)
{
	AXMatrix4 mat;
	...
	return AXCLS::OBBOBBOverlap(vExts, obb.Extents, mat);
}
...
/*	AABB-Sphere overlap test routine
	...
	fRadius: sphere's radius
*/
bool AXCLS::AABBSphereOverlap(const AXAABB& aabb, const AXVector3& vCenter, float fRadius)
{
	float d = 0.0f;
...
/*	OBB-Sphere overlap test routine
	...
	fRadius: sphere's radius
*/
bool AXCLS::OBBSphereOverlap(const AXOBB& obb, const AXVector3& vCenter, float fRadius)
{
	AXAABB aabb;
	aabb.Center.Clear();
	...
	AXVector3 vsc, vDelta(vCenter-obb.Center);
	vsc.x = DotProduct(obb.XAxis, vDelta);
	vsc.y = DotProduct(obb.YAxis, vDelta);
	vsc.z = DotProduct(obb.ZAxis, vDelta);

	return AXCLS::AABBSphereOverlap(aabb, vsc, fRadius);
}


abool AXCLS::RayToSphere(const AXVector3& vStart, const AXVector3& vDelta, const AXVector3& vSphereCenter, afloat32 fRadius, AXVector3& vHitPos, afloat32* pfFraction)
{
	if (pfFraction)
	{
		*pfFraction = 1.0f;
	...
	AXVector3 vSpherePointToStart = vStart - vSphereCenter;
	afloat32 fA = DotProduct(vDelta, vDelta);
	afloat32 fB = DotProduct(vSpherePointToStart, vDelta) * 2.0f;
	afloat32 fC = fRadius * fRadius - DotProduct(vSpherePointToStart, vSpherePointToStart);
	afloat32 fDeterminant = fB * fB + 4.0f * fA * fC;
	if (fDeterminant < 0.0f)
	{
		return afalse;
	}
	afloat32 fSqrtDeterminant = sqrtf(fDeterminant);
	afloat32 fT0 = (-fB + fSqrtDeterminant) * 0.5f / fA;
	afloat32 fT1 = (-fB - fSqrtDeterminant) * 0.5f / fA;
	if (fT0 <= 0.0f || fT1 >= 1.0f)
	{
		return afalse;
	}
