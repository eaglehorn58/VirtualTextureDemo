...
bool CDXPixelShader::Init(const char* szFile, const char* szFunc, CDXPixelShaderMate* pMate)
{
	//	Open shader file
	AFileWrapper file(szFile, "rb");
	if (!file.GetFile())
	{
		a_OutputLog("CDXPixelShader::Init, failed to open file [%s] !", szFile);
		return false;
	}
	...
	if (!pCodeBuf)
	{
		a_OutputLog("CDXPixelShader::Init, not enough memory [%d] for loading shader [%s] !", iFileLen, szFile);
		return false;
	}
	...
	HRESULT result = D3DCompile(pCodeBuf, iFileLen, NULL, NULL, &inc, szFunc, "ps_5_0",
	...
	if (FAILED(result))
	{
		if (pErrorMsg)
		{
			...
			a_OutputLog("CDXPixelShader::Init, compile error: %s !", sz);
		...
		{
			a_OutputLog("CDXPixelShader::Init, failed to compile shader [%s] !", szFile);
		}
	...
	result = pDevice->CreatePixelShader(pShaderBuf->GetBufferPointer(), pShaderBuf->GetBufferSize(), NULL, &m_pShader);
	if (FAILED(result))
	{
		pShaderBuf->Release();
		a_OutputLog("CDXPixelShader::Init, failed to call CreatePixelShader() for shader [%s] !", szFile);
		return false;
	}
	...
	if (pMate && pMate->GetConstBufNum())
	{
		...
		{
			pShaderBuf->Release();
			ASSERT(0 && "pixel shader constant buffer size dismatch !");
			a_OutputLog("CDXPixelShader::Init, shader constant buffer size dismatch [%s] !", szFile);
			return false;
		}
	}
	...
	if (pMate && pMate->GetConstBufNum())
	{
		...
		for (int i = 0; i < m_iCBufNum; i++)
		{
			...
			{
				Release();
				a_OutputLog("CDXPixelShader::Init, failed to call CreateBuffer() for shader [%s] !", szFile);
				return false;
			}
		}
	}
...
bool CDXPixelShader::CheckConstBufferSize(const void* pShaderBuf, int iShaderBufSize, CDXPixelShaderMate* pMate)
{
...
bool CDXPixelShader::UpdateParams(CDXPixelShaderMate* pMate, int slot)
{
	ASSERT(pMate);
	...
	if (slot >= 0)
	{
		...
		pDevContext->PSSetConstantBuffers(slot, 1, &m_aCBufs[slot].pCBuf);
	}
	else
	{
		...
		//	Commit buffer to device
		pDevContext->PSSetConstantBuffers(0, m_iCBufNum, aBufs);
	}

	return true;
}

bool CDXPixelShader::UpdateConstBuffer(CDXPixelShaderMate* pMate, int slot)
{
...
bool CDXPixelShader::Apply()
{
	...
	pDevContext->PSSetShader(m_pShader, NULL, 0);
